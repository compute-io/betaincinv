...
function temme_method_2_ibeta_inverse( z, r, theta ) {
	...
	co1 = [ -1, -5, 5 ];
	workspace[1] = -polyval( co1, s*s ) / (36 * sc_2);
	co2 = [ 1, 21, -69, 46 ];
	workspace[2] = polyval( co2, s*s ) / (1620 * sc_3);
	co3 = [ 7, -2, 33, -62, 31 ];
	workspace[3] = polyval( co3, s*s ) / (6480 * sc_4);
	co4 = [ 25, -52, -17, 88, -115, 46 ];
	workspace[4] = polyval( co4, s*s ) / (90720 * sc_5);
	terms[1] = polyval( workspace, eta0 );
	//
	// Now evaluate e2 and put it in terms[2]:
	//
	co5 = [ 7, 12, -78, 52 ];
	workspace[0] = -polyval( co5, s*s ) / (405 * sc_3);
	co6 = [ -7, 2, 183, -370, 185 ];
	workspace[1] = polyval( co6, s*s ) / (2592 * sc_4);
	co7 = [ -533, 776, -1835, 10240, -13525, 5410 ];
	workspace[2] = -polyval( co7, s*s ) / (204120 * sc_5);
	co8 = [ -1579, 3747, -3372, -15821, 45588, -45213, 15071 ];
	workspace[3] = -polyval( co8, s*s ) / (2099520 * sc_6);
	terms[2] = polyval( workspace, eta0 );
	//
	// And e3, and put it in terms[3]:
	//
	co9 = [ 449, -1259, -769, 6686, -9260, 3704 ];
	workspace[0] = polyval( co9, s*s ) / (102060 * sc_5);
	co10 = [ 63149, -151557, 140052, -727469, 2239932, -2251437, 750479 ];
	workspace[1] = -polyval( co10, s*s ) / (20995200 * sc_6);
	co11 = [ 29233, -78755, 105222, 146879, -1602610, 3195183, -2554139, 729754 ];
	workspace[2] = polyval( co11, s*s ) / (36741600 * sc_7);
	terms[3] = polyval( workspace, eta0 );
	//
	// Bring the correction terms together to evaluate eta,
	// this is the last equation on page 151:
	//
	eta = polyval(terms, 1/r );
	//
	...
	if( abs(eta) < 0.7) {
		...
		co12 = [ 1, -13, 13 ];
		workspace[3] = polyval( co12, workspace[0] ) / (36 * s * c);
		co13 = [ 1, 21, -69, 46 ];
		workspace[4] = polyval( co13, workspace[0] ) / (270 * workspace[0] * c * c);
		x = polyval( workspace, eta );
	} else {
		//
