...
function inverse_students_t( df, u, v, pol, pexact ) {
	...
	if( (floor(df) === df) && (df < 20) ) {
		...
		// cases first:
		//
		tolerance = ldexp( 1.0, (2 * policies::digits<T, Policy>()) / 3);
		...
		default:
			if ( df > 0x10000000 ) {
				result = - erfcinv( 2 * u ) * constants::root_two<T>();
				if((pexact) && (df >= 1e20))
					*pexact = true;
			} else if ( df < 3 ) {
				//
				...
				//
				crossover = ldexp(1.0f, iround(T(df / -0.654f), typename policies::normalise<Policy, policies::rounding_error<policies::ignore_error> >::type()));
				if( u > crossover ) {
		...
		if(df > 0x10000000) {
			...
			// The crossover point is roughly exponential in -df:
			//
			T crossover = ldexp(1.0f, iround( df / -0.654), typename policies::normalise<Policy, policies::rounding_error<policies::ignore_error> >::type()));
			if ( u > crossover ) {
				result = inverse_students_t_hill(df, u, pol);
			} else {
				result = inverse_students_t_tail_series(df, u, pol);
